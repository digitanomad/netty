...
public class EchoServerHandler extends ChannelInboundHandlerAdapter {
	...
	public void channelRead(ChannelHandlerContext ctx, Object msg)
		...
		// ByteBufAllocator의 buffer 메소드를 사용하여 생성된 바이트 버퍼는 ByteBufAllocator의 풀에서 관리되며
		// 바이트 버퍼를 채널에 기록하거나 명시적으로 release 메소드를 호출하면 바이트 버퍼 풀로 돌아간다.
		ByteBuf newBuffer = byteBufAllocator.buffer();
		
		// newBuffer 사용
		
		// write 메소드의 인수로 바이트 버퍼가 입력되면 데이터를 채널에 기록하고 난 뒤에 버퍼 풀로 돌아간다.
		ctx.write(msg);
	}
